		...
		<div class="container">
			...
			<template v-if="tickers.length">
				...
				<dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
					<div v-for="t_item in tickers" :key="t_item.name" @click="graph_title = t_item" :class="{
						'border-4':
							graph_title === t_item
						...
						<div class="px-4 py-5 sm:p-6 text-center">
							...
							<dd v-if="t_item.value" class="mt-1 text-3xl font-semibold text-gray-900">
								{{ t_item.value }}
							</dd>
						...
						<button @click.stop="remove(t_item.name)"
        ...
        >
          ...
          >
			...
			<section v-if="graph_title" class="relative">
				...
				<button @click="graph_title = null" type="button" class="absolute top-0 right-0">
...
export default {
	...
	methods: {
		...
		remove(name) {
			this.tickers = this.tickers.filter(elem => elem.name !== name)
			if (this.graph_title.name === name) {
		...
		fetchGraphData(newTicker) {
			setInterval(async () => {
				try {
					const response = await axios(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=5fe26ca8e9e8c0b1924366055147015e23c2e8ccf981f43c5dcd35bf81baa252`)
					const value = response.data.USD > 1 ? response.data.USD.toFixed(2) : response.data.USD.toPrecision(2)
					this.tickers.find(item => item.name === newTicker.name).value = value
				} catch (error) {
					console.log(error);
				}
			}, 5000)
